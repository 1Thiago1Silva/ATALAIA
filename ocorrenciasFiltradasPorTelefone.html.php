...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['telefone'])) {
    $telefoneBusca = trim($_POST['telefone']);
    $resultado = $ocorrencia->filtrarPorTelefone($telefoneBusca);
}
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <title>Filtrar por Telefone</title>
    <style>
        h3 {
            text-align: center;
            font-weight: bold;
...
<div class="container mt-4">
    <h3 class="mb-4">Filtrar por Telefone</h3>
    <div class="btn-group-custom">
        <!-- Formulário de busca por Telefone via POST -->
        <form method="POST" action="ocorrenciasFiltradasPorTelefone.html.php" class="d-flex">
            <input type="text" class="form-control" id="telefone" name="telefone" placeholder="Digite o telefone..." value="<?php echo htmlspecialchars($telefoneBusca); ?>" autocomplete="off">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        <?php
            ...
            echo "<p>Nenhuma ocorrência encontrada para o telefone informado.</p>";
        }
    }
    ?>
</div>
